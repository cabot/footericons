		...
		$(".has-fi-bg-select", $iconRow).each(function () {
			...
			$("select", $(this)).on("change", function () {
				if ($(this).val() == 0) {
					...
					$legendIcon.addClass("fa-stack-1x").removeClass("fa-stack-2x");
					if ($(this).val() == 1) {
						$legendIconBg.addClass("fa-circle").removeClass("fa-square");
					} else if ($(this).val() == 2) {
						$legendIconBg.addClass("fa-square").removeClass("fa-circle");
					}
				}
			});
		});
			...
			$(".fi-code-input").on("click", function () {
				if (!$(".fi-darkwrapper").length) {
					...
					icons.forEach(function (icon) {
						$(".fi-list").append('<li class="fi-item"><i title="' + icon + '" class="fa ' + icon + '" data-icode="' + icon + '"></i><span class="sr-only">' + icon + '</span></li>');
					});
